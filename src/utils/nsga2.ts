...
export async function runNSGA2(
  ...
  function decode(perm: number[], lf: number): number[][] {
    ...
    if (validPerm.length === 0) {
      console.warn(
        `[DECODE] EMPTY validPerm! perm=[${perm.slice(0, 5).join(',')}...](${perm.length}), n=${n}, locs=${locations.length}, lf=${lf.toFixed(3)}, constraint=${config.constraintValue}`,
      );
      return routes;
    }
      ...
      while (i < validPerm.length) {
        ...
        switch (config.constraintType) {
          ...
          case 'hours+visits': {
            ...
            if (
              driveH > config.constraintValue * lf ||
              prop.length > Math.round((config.maxVisits ?? 10) * lf)
            )
              violation = true;
            break;
          }
        ...
        if (violation) break;
        day.push(validPerm[i]);
        i++;
      }
      if (day.length === 0 && i < validPerm.length) {
        day.push(validPerm[i]);
        i++;
      }
      if (day.length > 0) {
  ...
  function computeObjectives(routes: number[][]): [number, number] {
    return [
      routes.reduce((s, r) => s + routeDist(r), 0),
      Math.max(...routes.map((r) => routeHours(r)), 0),
    ];
  }
  ...
  function initPopulation(): Individual[] {
    ...
    for (let i = 0; i < POP; i++) {
      ...
      pop.push({
        ...
        loadFactor: lf,
        routes,
        objectives: computeObjectives(routes),
        rank: 0,
        crowdingDist: 0,
      });
    }
  ...
  function crowdingDistance(pop: Individual[], front: number[]): void {
    ...
    for (const objIdx of [0, 1]) {
      ...
      for (let i = 1; i < m - 1; i++)
        pop[front[i]].crowdingDist +=
          (pop[front[i + 1]].objectives[objIdx] - pop[front[i - 1]].objectives[objIdx]) / range;
    }
  }
  ...
  function tournamentSelect(pop: Individual[]): Individual {
    ...
    const a = pop[i],
      b = pop[j];
    const better =
      a.rank < b.rank || (a.rank === b.rank && a.crowdingDist > b.crowdingDist) ? a : b;
  ...
  function orderCrossover(p1: number[], p2: number[]): [number[], number[]] {
    ...
    const ch1 = new Array(len).fill(-1),
      ch2 = new Array(len).fill(-1);
  ...
  function routesToDayRoutes(routes: number[][]): DayRoute[] {
    return routes.map((indices, di) => {
      ...
      for (let i = 0; i < indices.length; i++) {
        ...
        cd += d;
        ct += d / config.avgSpeed + config.visitTime / 60;
        stops.push({
          sequence: i + 1,
  ...
  for (let gen = 0; gen < GENS; gen++) {
    ...
    for (const f of fronts) {
      if (f.length > 0) crowdingDistance(combined, f);
    }
    ...
    while (r < fronts.length && next.length + fronts[r].length <= POP) {
      ...
      for (let fi = 0; fi < f.length; fi++) {
        ...
        next.push({
          ...combined[idx],
          perm: [...combined[idx].perm],
          routes: combined[idx].routes.map((x) => [...x]),
        });
  ...
  const balanced = pareto.reduce((best, ind) => {
    ...
    return score < (best.objectives[0] - idealDist) / rDist + (best.objectives[1] - idealDur) / rDur
      ? ind
      : best;
  });

  console.log(
    `${FLOW}   NSGA2 done: ${Date.now() - tNsga}ms, pareto=${front.length} solutions, balanced=${balanced.objectives[0].toFixed(1)}km/${balanced.objectives[1].toFixed(2)}h`,
  );
