...
export function useLeafletRoutes(
  ...
  const poiDataRef = useRef(
    new Map<string, { lat: number; lng: number; day: number; name: string }>(),
  );
  ...
  useEffect(() => {
    ...
    for (const day of options.routes) {
      try {
        ...
        const isDimmed =
          options.highlightDay !== null && options.highlightDay !== undefined && !isHighlighted;
        ...
        console.log(
          `[Route] Day ${day.day}: usedRoadGeo=${usedRoadGeo}, source=${daySource ?? 'none'}, coords=${coords.length}, isEstimated=${!usedRoadGeo || daySource === 'haversine' || daySource === undefined}`,
        );
    ...
    const spatialKey = JSON.stringify(
      options.routes.map((d) => `${d.day}:${d.stops.length}`).join(','),
    );
  ...
  useEffect(() => {
    ...
    for (const [, group] of groups) {
      group.eachLayer((layer) => {
        const poiData = (layer as any)._poiData as
