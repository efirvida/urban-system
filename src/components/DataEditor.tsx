...
export default function DataEditor({ rows, onChange, onConfirm, onBack }: DataEditorProps) {
  ...
  const filteredRows = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      result = result.filter(
        (r) => r.name.toLowerCase().includes(q) || r.rawLat.includes(q) || r.rawLng.includes(q),
      );
    }
  ...
  const handleToggleRow = useCallback(
    (id: string) => {
      onChange(rows.map((r) => (r.id === id ? { ...r, selected: !r.selected } : r)));
    },
    [rows, onChange],
  );
  ...
  return (
    <div className="flex flex-col h-full gap-3">
      ...
      <div className="text-xs text-gray-400">
        Mostrando {filteredRows.length} de {rows.length} filas &middot; {selectedCount}{' '}
        seleccionadas
      </div>
            ...
            {filteredRows.length === 0 && (
              <tr>
                <td colSpan={5} className="p-6 text-center text-sm text-gray-400">
                  {search ? t('dataEditor.empty.noResults') : t('dataEditor.empty.noRows')}
                </td>
              </tr>
            )}
            {filteredRows.map((row) => (
              ...
              >
                ...
                <td className="p-1.5">
                  <input
                    value={row.rawLat}
                    onChange={(e) => handleEditField(row.id, 'lat', e.target.value)}
                    className={cn(
                ...
                <td className="p-1.5">
                  <input
                    value={row.rawLng}
                    onChange={(e) => handleEditField(row.id, 'lng', e.target.value)}
                    className={cn(
                      'w-full bg-transparent border-b border-transparent focus:border-blue-500 focus:outline-none px-1 py-0.5 font-mono text-xs',
                ...
                <td className="p-1.5 pr-2">
                  {row.validationError ? (
                    <span className="text-red-500 cursor-help" title={row.validationError}>
                    ...
                    <span className="text-green-500">
                      <Check
                        ...
                        aria-label={t('dataEditor.ariaLabels.validRow')}
                      />
                    </span>
                  )}
                </td>
      ...
      <div className="flex items-center justify-between border-t pt-3">
        ...
        >
          {selectedCount > 0 ? `Confirmar ${selectedCount} →` : t('dataEditor.selectionHint')}
        </button>
      </div>
    </div>
  );
