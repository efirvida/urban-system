    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <button
          ...
          aria-label={
            placingHome
              ? t('configPanel.ariaLabels.cancelPlaceHome')
              : t('configPanel.ariaLabels.placeHome')
          }
          className={cn(
        ...
        {config.homeLat !== 0 && config.homeLng !== 0 && (
          <p className="text-xs text-green-600 mt-1 inline-flex items-center gap-1">
            ...
            {t('configPanel.labels.homeAt', {
              lat: config.homeLat.toFixed(4),
              lng: config.homeLng.toFixed(4),
            })}
          </p>
        )}
      </div>
          ...
          <>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-2 text-center">
                {t('configPanel.labels.workDay')}
              </label>
              <div className="flex items-center justify-center gap-4">
                <button
                  type="button"
                  onClick={() =>
                    update({ constraintValue: Math.max(1, config.constraintValue - 0.5) })
                  }
                ...
                <div className="text-center min-w-[80px]">
                  <div className="text-3xl font-bold text-gray-900">
                    {config.constraintValue.toFixed(1)}
                  </div>
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-2 text-center">
                {t('configPanel.constraintTypes.visits')}
              </label>
              <div className="flex items-center justify-center gap-4">
                <button
                  type="button"
          ...
          <>
            <label className="block text-xs font-medium text-gray-600 mb-2 text-center">
              {config.constraintType === 'hours'
                ? t('configPanel.labels.workDay')
                : t('configPanel.labels.visitsPerDay')}
            </label>
            <div className="flex items-center justify-center gap-4">
              <button
                type="button"
                onClick={() =>
                  update({
                    constraintValue: Math.max(
                      1,
                      config.constraintValue - (config.constraintType === 'hours' ? 0.5 : 1),
                    ),
                  })
                }
                aria-label={
                  config.constraintType === 'hours'
                    ? t('configPanel.ariaLabels.decreaseHours')
                    : t('configPanel.ariaLabels.decreaseVisits')
                }
                className="w-10 h-10 rounded-full bg-white border border-gray-300 text-gray-600 hover:bg-gray-100 flex items-center justify-center transition-colors"
              >
                <Minus className="w-4 h-4" />
              ...
              <div className="text-center min-w-[80px]">
                ...
                <div className="text-xs text-gray-400 mt-0.5">
                  {config.constraintType === 'hours' ? 'horas' : 'paradas'}
                </div>
              </div>
              <button
                type="button"
                ...
                aria-label={
                  config.constraintType === 'hours'
                    ? t('configPanel.ariaLabels.increaseHours')
                    : t('configPanel.ariaLabels.increaseVisits')
                }
                className="w-10 h-10 rounded-full bg-white border border-gray-300 text-gray-600 hover:bg-gray-100 flex items-center justify-center transition-colors"
              >
                <Plus className="w-4 h-4" />
          ...
          <div>
            <div className="flex justify-between text-xs text-gray-500 mb-1">
              ...
              <span className="font-medium text-gray-700">
                {(config.visitTime / 60).toFixed(1)} h
              </span>
            </div>
            <input
              type="range"
