...
Source of truth for the `localization` capability: i18n infrastructure, locale persistence, locale switcher, the `t()` translation contract, locale-aware formatters, pluralization, and locale JSON parity.
...
`tsc --noEmit`, `next lint`, and `next build` all pass. The 22 touched files contain zero hardcoded user-facing Spanish OR Portuguese strings. The `LocaleSwitcher` toggles between `pt-BR` and `es` and the choice persists across refresh. `formatDistance` and `formatDuration` produce locale-correct output. Pluralization works in both locales. Missing keys are visible in dev and fall back to pt-BR in production.
